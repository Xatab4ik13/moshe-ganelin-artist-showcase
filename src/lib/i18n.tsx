import { createContext, useContext, useEffect, useState, type ReactNode } from "react";

export type Lang = "en" | "es" | "pt";

export const langOptions: { code: Lang; label: string }[] = [
  { code: "en", label: "EN" },
  { code: "es", label: "ES" },
  { code: "pt", label: "POR" },
];

export const dict = {
  en: {
    navHome: "Home",
    navAbout: "About",
    navBio: "Bio",
    navPublications: "Publications",
    navMusic: "Music",
    navOrgan: "Organ",
    navOrchestra: "Orchestra",
    navPiano: "Piano",
    navTranscriptions: "Transcriptions",
    navRecordings: "Recordings",
    navPoetry: "Poetry",
    navConcerts: "Concerts",
    navGallery: "Gallery",
    navContact: "Contact",
    langRussian: "Russian",
    langEnglish: "English",
    langSpanish: "Spanish",
    langPortuguese: "Portuguese",
    openMenu: "Open menu",
    closeMenu: "Close menu",
    openMenuShort: "Menu",
    mainNav: "Main navigation",
    homeUpcoming: "Upcoming concerts",
    homeAllConcerts: "Full calendar & archive",
    programNote: "Program — sample text.",
    concertsTitle: "Concerts",
    concertsLead: "A catalogue of upcoming performances and an archive of past evenings — sample text.",
    concertsPast: "Past performances",
    aboutTitle: "Biography",
    aboutLead: "A short introduction to the musician — sample text, to be replaced with the final version.",
    aboutMilestones: "Milestones",
    aboutPublications: "Publications",
    musicTitle: "Compositions & Recordings",
    musicLead: "Works by discipline, with links to recordings and scores — sample text.",
    musicOrgan: "Organ works",
    musicOrchestra: "Orchestral & chamber",
    musicPiano: "Piano music",
    musicTranscriptions: "Transcriptions & improvisations",
    musicRecordings: "Recordings",
    sectionDescription: "Section description — sample text.",
    poetryIntroTitle: "On the poetry",
    poetryIntroText: "Introductory text about the poetry — sample text, to be replaced with the final version.",
    contactsFollow: "Follow Moshe Ariel Ganelin",
    poetryTitle: "Ganelin's poetry",
    poetryLead: "Poems in four languages — sample text, to be replaced with the final versions.",
    galleryTitle: "Gallery",
    galleryLead: "Photo captions — sample text, to be replaced.",
    contactsTitle: "Contact",
    contactsLead: "Contact directions below. Texts and addresses will be replaced with the final ones.",
    contactsBooking: "Concerts & booking",
    contactsPress: "Press",
    contactsScores: "Scores & publishing",
    contactsManagement: "Management",
    contactsManagementText: "Text about management and booking terms — sample, to be replaced.",
    blockNote: "Description — sample text.",
    footerBooking: "Concerts and booking — sample text",
    scorePdf: "Score (PDF)",
    fullScore: "Full score",
    poemSampleTitle: "Poem title — sample",
    poemSampleText: "Poem text — sample, to be replaced with the final version.",
    navBiography: "Biography",
    navPress: "Press",
    navGanelinMusic: "Ganelin's music",
    navGanelinPoetry: "Ganelin's poetry",
    navImprovisations: "Improvisations",
    navConcertsVideo: "Concert videos",
    navSearch: "Search",
    searchPlaceholder: "Search the site",
    searchNoResults: "Nothing found",
    searchHint: "Type a name, a work or a section",
    homePress: "In the press",
    homePressAll: "All press materials",
    homeVideo: "YouTube & live streams",
    homeVideoAll: "YouTube channel",
    homeWatch: "Watch",
    musicIntroTitle: "On the music",
    musicIntroText: "Introductory text about the composer's work — sample text, to be replaced with the final version.",
    listOfWorks: "List of Works",
    catSymphonic: "Symphonic music",
    catOrgan: "Organ music",
    catVocal: "Vocal music",
    catChoir: "Choir music",
    catChamber: "Chamber music",
    workBack: "Back to the list of works",
    workAbout: "About the work",
    workQuotes: "Quotes",
    workVideo: "Video",
    workDetails: "Details",
    workPremiere: "Premiere",
    workDuration: "Duration",
    workScoring: "Scoring",
    improvisationsTitle: "Improvisations",
    improvisationsLead: "Improvisation as a living form of composition — sample text, to be replaced.",
    transcriptionsTitle: "Transcriptions",
    transcriptionsLead: "Transcriptions for organ and piano — sample text, to be replaced.",
    concertsVideoTitle: "Concert videos",
    concertsVideoLead: "Recordings of concerts and live streams — sample text, to be replaced.",
    pressTitle: "Press",
    pressLead: "Reviews, interviews and press materials — sample text, to be replaced.",
    pressQuotes: "In the press",
    pressQuoteText: "Review quote — sample text. A line from a critic will be placed here.",
    pressQuoteSource: "Publication name",
    readMore: "Read",
    concertBack: "Back to all concerts",
    concertWhen: "Date",
    concertWhere: "Venue",
    concertProgram: "Programme",
    concertAbout: "About the concert",
    concertVideo: "Video",
    concertDetailsText: "Concert description — sample text, to be replaced with the final version.",
    concertTickets: "Tickets & booking",
    concertsCalendar: "Calendar",
    navConcertSchedule: "Concert schedule",
  },
  es: {
    navHome: "Inicio",
    navAbout: "Biografía",
    navBio: "Bio",
    navPublications: "Publicaciones",
    navMusic: "Música",
    navOrgan: "Órgano",
    navOrchestra: "Orquesta",
    navPiano: "Piano",
    navTranscriptions: "Transcripciones",
    navRecordings: "Grabaciones",
    navPoetry: "Poesía",
    navConcerts: "Conciertos",
    navGallery: "Galería",
    navContact: "Contacto",
    langRussian: "Ruso",
    langEnglish: "Inglés",
    langSpanish: "Español",
    langPortuguese: "Portugués",
    openMenu: "Abrir menú",
    closeMenu: "Cerrar menú",
    openMenuShort: "Menú",
    mainNav: "Navegación principal",
    homeUpcoming: "Próximos conciertos",
    homeAllConcerts: "Calendario completo y archivo",
    programNote: "Programa — texto de ejemplo.",
    concertsTitle: "Conciertos",
    concertsLead: "Catálogo de próximas actuaciones y archivo de veladas pasadas — texto de ejemplo.",
    concertsPast: "Actuaciones pasadas",
    aboutTitle: "Biografía",
    aboutLead: "Breve introducción al músico — texto de ejemplo, será reemplazado por la versión final.",
    aboutMilestones: "Etapas del camino",
    aboutPublications: "Publicaciones",
    musicTitle: "Composiciones y grabaciones",
    musicLead: "Obras por disciplina, con enlaces a grabaciones y partituras — texto de ejemplo.",
    musicOrgan: "Obras para órgano",
    musicOrchestra: "Orquestal y de cámara",
    musicPiano: "Música para piano",
    musicTranscriptions: "Transcripciones e improvisaciones",
    musicRecordings: "Grabaciones",
    sectionDescription: "Descripción de la sección — texto de ejemplo.",
    poetryIntroTitle: "Sobre la poesía",
    poetryIntroText: "Texto introductorio sobre la poesía — texto de ejemplo, será reemplazado.",
    contactsFollow: "Sigue a Moshe Ariel Ganelin",
    poetryTitle: "Poesía de Ganelin",
    poetryLead: "Poemas en cuatro idiomas — texto de ejemplo, será reemplazado por las versiones finales.",
    galleryTitle: "Galería",
    galleryLead: "Pies de foto — texto de ejemplo, será reemplazado.",
    contactsTitle: "Contacto",
    contactsLead: "A continuación, las vías de contacto. Los textos y direcciones serán reemplazados por los definitivos.",
    contactsBooking: "Conciertos y contratación",
    contactsPress: "Prensa",
    contactsScores: "Partituras y ediciones",
    contactsManagement: "Management",
    contactsManagementText: "Texto sobre management y condiciones de contratación — ejemplo, será reemplazado.",
    blockNote: "Descripción — texto de ejemplo.",
    footerBooking: "Conciertos y contratación — texto de ejemplo",
    scorePdf: "Partitura (PDF)",
    fullScore: "Partitura completa",
    poemSampleTitle: "Título del poema — ejemplo",
    poemSampleText: "Texto del poema — ejemplo, será reemplazado por la versión final.",
    navBiography: "Biografía",
    navPress: "Prensa",
    navGanelinMusic: "Música de Ganelin",
    navGanelinPoetry: "Poesía de Ganelin",
    navImprovisations: "Improvisaciones",
    navConcertsVideo: "Vídeos de conciertos",
    navSearch: "Buscar",
    searchPlaceholder: "Buscar en el sitio",
    searchNoResults: "No se ha encontrado nada",
    searchHint: "Escribe un nombre, una obra o una sección",
    homePress: "En la prensa",
    homePressAll: "Todos los materiales de prensa",
    homeVideo: "YouTube y transmisiones",
    homeVideoAll: "Canal de YouTube",
    homeWatch: "Ver",
    musicIntroTitle: "Sobre la música",
    musicIntroText: "Texto introductorio sobre la obra del compositor — texto de ejemplo, será reemplazado.",
    listOfWorks: "Lista de obras",
    catSymphonic: "Música sinfónica",
    catOrgan: "Música para órgano",
    catVocal: "Música vocal",
    catChoir: "Música coral",
    catChamber: "Música de cámara",
    workBack: "Volver a la lista de obras",
    workAbout: "Sobre la obra",
    workQuotes: "Citas",
    workVideo: "Vídeo",
    workDetails: "Detalles",
    workPremiere: "Estreno",
    workDuration: "Duración",
    workScoring: "Plantilla",
    improvisationsTitle: "Improvisaciones",
    improvisationsLead: "La improvisación como forma viva de composición — texto de ejemplo.",
    transcriptionsTitle: "Transcripciones",
    transcriptionsLead: "Transcripciones para órgano y piano — texto de ejemplo.",
    concertsVideoTitle: "Vídeos de conciertos",
    concertsVideoLead: "Grabaciones de conciertos y transmisiones — texto de ejemplo.",
    pressTitle: "Prensa",
    pressLead: "Reseñas, entrevistas y materiales de prensa — texto de ejemplo.",
    pressQuotes: "En la prensa",
    pressQuoteText: "Cita de reseña — texto de ejemplo. Aquí irá una línea de un crítico.",
    pressQuoteSource: "Nombre de la publicación",
    readMore: "Leer",
    concertBack: "Volver a todos los conciertos",
    concertWhen: "Fecha",
    concertWhere: "Sala",
    concertProgram: "Programa",
    concertAbout: "Sobre el concierto",
    concertVideo: "Vídeo",
    concertDetailsText: "Descripción del concierto — texto de ejemplo, se sustituirá por la versión final.",
    concertTickets: "Entradas y reservas",
    concertsCalendar: "Calendario",
    navConcertSchedule: "Cartelera de conciertos",
  },
  pt: {
    navHome: "Início",
    navAbout: "Sobre",
    navBio: "Bio",
    navPublications: "Publicações",
    navMusic: "Música",
    navOrgan: "Órgão",
    navOrchestra: "Orquestra",
    navPiano: "Piano",
    navTranscriptions: "Transcrições",
    navRecordings: "Gravações",
    navPoetry: "Poesia",
    navConcerts: "Concertos",
    navGallery: "Galeria",
    navContact: "Contato",
    langRussian: "Russo",
    langEnglish: "Inglês",
    langSpanish: "Espanhol",
    langPortuguese: "Português",
    openMenu: "Abrir menu",
    closeMenu: "Fechar menu",
    openMenuShort: "Menu",
    mainNav: "Navegação principal",
    homeUpcoming: "Próximos concertos",
    homeAllConcerts: "Calendário completo e arquivo",
    programNote: "Programa — texto de exemplo.",
    concertsTitle: "Concertos",
    concertsLead: "Catálogo das próximas atuações e arquivo de noites passadas — texto de exemplo.",
    concertsPast: "Atuações passadas",
    aboutTitle: "Biografia",
    aboutLead: "Breve introdução ao músico — texto de exemplo, será substituído pela versão final.",
    aboutMilestones: "Etapas do caminho",
    aboutPublications: "Publicações",
    musicTitle: "Composições e gravações",
    musicLead: "Obras por disciplina, com links para gravações e partituras — texto de exemplo.",
    musicOrgan: "Obras para órgão",
    musicOrchestra: "Orquestral e de câmara",
    musicPiano: "Música para piano",
    musicTranscriptions: "Transcrições e improvisações",
    musicRecordings: "Gravações",
    sectionDescription: "Descrição da seção — texto de exemplo.",
    poetryIntroTitle: "Sobre a poesia",
    poetryIntroText: "Texto introdutório sobre a poesia — texto de exemplo, será substituído.",
    contactsFollow: "Siga Moshe Ariel Ganelin",
    poetryTitle: "Poesia de Ganelin",
    poetryLead: "Poemas em quatro idiomas — texto de exemplo, será substituído pelas versões finais.",
    galleryTitle: "Galeria",
    galleryLead: "Legendas das fotos — texto de exemplo, será substituído.",
    contactsTitle: "Contato",
    contactsLead: "Abaixo, os canais de contato. Textos e endereços serão substituídos pelos finais.",
    contactsBooking: "Concertos e contratação",
    contactsPress: "Imprensa",
    contactsScores: "Partituras e edições",
    contactsManagement: "Management",
    contactsManagementText: "Texto sobre management e condições de contratação — exemplo, será substituído.",
    blockNote: "Descrição — texto de exemplo.",
    footerBooking: "Concertos e contratação — texto de exemplo",
    scorePdf: "Partitura (PDF)",
    fullScore: "Partitura completa",
    poemSampleTitle: "Título do poema — exemplo",
    poemSampleText: "Texto do poema — exemplo, será substituído pela versão final.",
    navBiography: "Biografia",
    navPress: "Imprensa",
    navGanelinMusic: "Música de Ganelin",
    navGanelinPoetry: "Poesia de Ganelin",
    navImprovisations: "Improvisações",
    navConcertsVideo: "Vídeos de concertos",
    navSearch: "Buscar",
    searchPlaceholder: "Buscar no site",
    searchNoResults: "Nada encontrado",
    searchHint: "Digite um nome, uma obra ou uma seção",
    homePress: "Na imprensa",
    homePressAll: "Todos os materiais de imprensa",
    homeVideo: "YouTube e transmissões",
    homeVideoAll: "Canal do YouTube",
    homeWatch: "Assistir",
    musicIntroTitle: "Sobre a música",
    musicIntroText: "Texto introdutório sobre a obra do compositor — texto de exemplo, será substituído.",
    listOfWorks: "Lista de obras",
    catSymphonic: "Música sinfônica",
    catOrgan: "Música para órgão",
    catVocal: "Música vocal",
    catChoir: "Música coral",
    catChamber: "Música de câmara",
    workBack: "Voltar à lista de obras",
    workAbout: "Sobre a obra",
    workQuotes: "Citações",
    workVideo: "Vídeo",
    workDetails: "Detalhes",
    workPremiere: "Estreia",
    workDuration: "Duração",
    workScoring: "Formação",
    improvisationsTitle: "Improvisações",
    improvisationsLead: "A improvisação como forma viva de composição — texto de exemplo.",
    transcriptionsTitle: "Transcrições",
    transcriptionsLead: "Transcrições para órgão e piano — texto de exemplo.",
    concertsVideoTitle: "Vídeos de concertos",
    concertsVideoLead: "Gravações de concertos e transmissões — texto de exemplo.",
    pressTitle: "Imprensa",
    pressLead: "Críticas, entrevistas e materiais de imprensa — texto de exemplo.",
    pressQuotes: "Na imprensa",
    pressQuoteText: "Citação de crítica — texto de exemplo. Uma frase de um crítico ficará aqui.",
    pressQuoteSource: "Nome da publicação",
    readMore: "Ler",
    concertBack: "Voltar a todos os concertos",
    concertWhen: "Data",
    concertWhere: "Local",
    concertProgram: "Programa",
    concertAbout: "Sobre o concerto",
    concertVideo: "Vídeo",
    concertDetailsText: "Descrição do concerto — texto de exemplo, a substituir pela versão final.",
    concertTickets: "Bilhetes e reservas",
    concertsCalendar: "Calendário",
    navConcertSchedule: "Agenda de concertos",
  },
} as const;

export type DictKey = keyof (typeof dict)["en"];

/** Тексты, изменённые в админ-панели: ключ -> язык -> текст. */
export type TextOverrides = Partial<Record<string, Partial<Record<Lang, string>>>>;

type LangContextValue = { lang: Lang; setLang: (lang: Lang) => void; t: (key: DictKey) => string };

const LangContext = createContext<LangContextValue>({
  lang: "en",
  setLang: () => {},
  t: (key) => dict.en[key],
});

export function LanguageProvider({
  children,
  overrides,
}: {
  children: ReactNode;
  overrides?: TextOverrides;
}) {
  const [lang, setLangState] = useState<Lang>("en");


  useEffect(() => {
    const stored = window.localStorage.getItem("mg-lang");
    if (stored === "en" || stored === "es" || stored === "pt") setLangState(stored);
  }, []);

  const setLang = (next: Lang) => {
    setLangState(next);
    window.localStorage.setItem("mg-lang", next);
  };

  useEffect(() => {
    document.documentElement.lang = lang;
  }, [lang]);

  const t = (key: DictKey) => dict[lang][key];

  return <LangContext.Provider value={{ lang, setLang, t }}>{children}</LangContext.Provider>;
}

export function useLanguage() {
  return useContext(LangContext);
}
